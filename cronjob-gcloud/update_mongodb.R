# Code to update MongoDB Atlas with data from KoboToolbox
...
user_kobo <- "user_name"
password_kobo <- "passwords"

# Credentials for MongoDB Atlas (adjust for each project)
user_mongodb <- "user_name"
password_mongodb <- "passwords"
...
# MongoDB connection string (replace according to the project)
mongo_url <- paste0(
  "mongodb+srv://", user_mongodb, ":", password_mongodb,
  "@cluster0.0g68cv4.mongodb.net/?retryWrites=true&w=majority"
)
...
################################
# Upload kobo data to MongoDB Atlas
...
if (result$nInserted > 0) {
  print(
    paste("Data has been successfully inserted into",
          df_name, "collection. UUID:", uuid)
  )
} else {
  print(
    paste("An error occurred while inserting data into", 
          df_name, "collection. UUID:", uuid)
  )
} else {
  print(
    paste("UUID already exists in",
          df_name, "collection. No insertion performed. UUID:", uuid)
  )
}

# Clean up environment and memory
rm(df, i, form_title, auth)
invisible(gc())